<template>
  ...
  >
    ...
    <div v-if="!$slots.icon && !icon" class="d-flex align-items-center">
      <b-spinner small class="mr-2" v-if="processing"></b-spinner>
      <slot></slot>
    </div>
...
export default {
  ...
  props: {
    ...
    href: {
      type: String,
      default: '',
      description: "Add href tag to the tag"
    },
    processing: {
      type: Boolean,
      default: false,
  ...
  computed: {
    classes() {
      let btnClasses = [
        ...
        this.tag === 'button' ? {"disabled": this.processing} : "",
